...
const DocumentoCard = ({ documento, onView, onDownload, onDuplicate, onDelete }) => {
  const getIconByType = (tipo) => {
    switch (tipo) {
      ...
      case 'protocolo':
        return <ClipboardDocumentListIcon className="w-8 h-8 text-teal-600" />;
      case 'normativa':
        return <ChartBarIcon className="w-8 h-8 text-green-600" />;
      default:
        return <FolderIcon className="w-8 h-8 text-gray-600" />;
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              {documento.version && (
                <span className="text-xs text-gray-500">v{documento.version}</span>
              )}
      ...
      {documento.tags && documento.tags.length > 0 && (
        <div className="flex items-center gap-1 mb-3 flex-wrap">
          ...
          {documento.tags.length > 3 && (
            <span className="text-xs text-gray-500">+{documento.tags.length - 3}</span>
          )}
        </div>
      )}
      ...
      <div className="flex gap-2 pt-3 border-t border-gray-100">
        ...
        {documento.archivo_url && (
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1 px-3 py-2 text-sm text-teal-600 hover:bg-teal-50 rounded transition-colors"
            title="Descargar"
          >
        ...
        <button
          ...
          className="flex items-center justify-center px-3 py-2 text-sm text-purple-600 hover:bg-purple-50 rounded transition-colors"
          title="Duplicar"
        >
